...
## Logik und Algebra
### Mathematische Beweisemethoden
1. Berechnung aller Fälle (z.B. Wahrheitstafel)
2. Ein(!) Gegenbeispiel
3. Direkter Beweis: Voraussetzung: A ist wahr. Wir zeigen A ∆ B
Dann folgt: B ist wahr (Abtrennungsregel)
4. Indirekter Beweis: Voraussetzung: A ist wahr. Wir zeigen ¬B ∆ ¬A.
Dann folgt (A ∆ B) und damit folgt: B ist wahr
5. Widerspruchsbeweis: Voraussetzung: A ist wahr. Annahme: ¬B ist wahr.
Wir zeigen (A · ¬B) ist falsch (führt zum Widerspruch).
Dann folgt ¬(A · ¬B) ist wahr, also (A ∆ B) und damit folgt: B ist wahr
6. Vollständige Induktion
7. Fallunterscheidung: In Fälle aufteilen und für jeden Fall eine Beweismethode wählen
### Vollständige Induktion
1. Induktionsanfang: n=1: Zeige A(1) ist wahr
2. Induktionsschritt: n æ n+1: Zeige ’n œ N: (A(n) ∆ A(n+1))
3. Induktionsschluß: A(1) wahr und (A(n) ∆ A(n + 1)) wahr für beliebiges n œ N, folgt mit wiederholter Anwendung der Abtrennungsregel:
    A(1) wahr, A(2) wahr, A(3) wahr, ... , A(n) wahr ( ’n œ N)
### Mengenoperationen
![Mengenoperationen](img/s1_logik_mengenoperationen.png)
### Cartesisches Produkt
Das Cartesische Produkt ist die Menge aller möglichen Paare aus den Elementen der beiden Mengen.
### Äquivalenzrelation
Eine Äquivalenzrelation ist eine Relation, die reflexiv, symmetrisch und transitiv ist.
Sei R eine Äquivalenzrelation in einer Menge M.
1. Für jedes m œ M gilt m œ [m].
2. Ist m äquivalent zu n also m ≥ n, dann gilt n œ [m] und m œ [n].
3. Ist [n] fl [m] ”= ÿ, dann folgt daraus [n]=[m].
### Abbildungen
![Abbildungen](img/s1_mathe_abbildungen.png)
